...
这是一个可独立接入 SwiftUI 的黑客松模块：实时语音陪练（MiniMax）、器械识别（Kimi）和本地训练记忆（SwiftData）。没有新的 iOS 三方包，也不需要 CloudKit 或数据库后端。

## 1. 接入方式

1. 保留现有 `CoachAPI` / Cloudflare SSE 文本路径；本模块不替换它。
2. 将 `vance-gateway/` 作为独立 Node 服务部署，并通过 HTTPS/WSS 暴露给 App。
...
4. `WorkoutSession` 已负责将 `RealtimeCoachClient`、`GymVisionAPI` 和本地 `WorkoutStore` 接到 `CoachThread`；Home 对话页已提供按住说话与拍照入口，无需再交付单独 UI。
...
| Gateway Secret | `MINIMAX_API_KEY`、`KIMI_API_KEY`、`VANCE_GATEWAY_SHARED_SECRET` | 仅部署环境；绝不进 iOS 包或 Git |
| iOS 权限 | 麦克风、语音识别、相机、使用期间定位 | 现有 `Info.plist` 已有说明 |

Gateway 路由：

- `GET /realtime`：MiniMax 实时语音 WebSocket。
...
真机验收：按住说话可听到实时回复；拍摄器械后只出现 high 设备、地点与合并后的本地记忆。若需要排查时延，可读取本机 `GymVisionTimingRecord`，无需向用户展示诊断数字。
